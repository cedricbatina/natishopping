...
import java.annotation.Resource;
...
@EJB
private FacadeProduit facadeProdui:
 @EJB
 ...
 for (ItemPanier scItem : items) {
  ...
  ProduitCommandePK orderedProduckPK = new ProduitCommandePK();
orderedProductPK.setCommandeClientId(order.getId()) ;
orderedProduckPK.setProduitId(productId).
ProduitCommande orderedItem =  new ProduitCommande(orderedProduckPK);
...
em.persist (orderedItem);
 ...
 public Map getDetailsCde(int idCde) {
  ...
  List <ProduitCommande> produitsCommandes = facadeProduitCommande.findByOrderedId(idCde);
  List <Produit> produits = new ArrayList<Produit>();
  for (ProduitCommande op : produitsCommandes) {
   Produit p = (Produit) facadeProduit.find(op.getProduitCommandePK().getProductId());
   produits.add(p);
